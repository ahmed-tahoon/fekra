...
// SplashCursor — WebGL fluid-simulation cursor (Navier–Stokes solver).
// Port of Pavel Dobryakov's WebGL-Fluid-Simulation (MIT), reactbits style.
...
export function SplashCursor({
  ...
  useEffect(() => {
    ...
    function splat(x: number, y: number, dx: number, dy: number, color: any) {
      ...
      gl.uniform1f(splatProgram.uniforms.aspectRatio, (canvas as HTMLCanvasElement).width / (canvas as HTMLCanvasElement).height);
    ...
    function onTouchMove(e: TouchEvent) {
      ...
      for (let i = 0; i < touches.length; i++) {
        const t = touches[i]!;
        const posX = scaleByPixelRatio(t.clientX);
        const posY = scaleByPixelRatio(t.clientY);
    ...
    function onTouchStart(e: TouchEvent) {
      ...
      for (let i = 0; i < touches.length; i++) {
        ...
        const posX = scaleByPixelRatio(t.clientX);
        const posY = scaleByPixelRatio(t.clientY);
        updatePointerDownData(pointer, t.identifier, posX, posY);
      }
    }
